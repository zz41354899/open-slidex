  ...
  return (
    <header className={styles.header} data-editor-header-variant={variant}>
      ...
      <div className={styles.headerRight}>
        ...
        {onReplay ? (
          <button aria-label="Replay animations" className={styles.actionButton} onClick={onReplay} title="Replay animations" type="button">
            <Redo2 size={15} /><span>Replay</span>
          </button>
        ) : null}
        <div className={styles.exportControl} ref={(node) => {
          ...
          {exportInteraction === "split" && exportOptions?.length ? <button aria-expanded={exportOpen} aria-label="Choose export format" className={styles.exportChevron} onClick={() => setExportOpen((value) => !value)} type="button"><ChevronDown size={14} /></button> : null}
