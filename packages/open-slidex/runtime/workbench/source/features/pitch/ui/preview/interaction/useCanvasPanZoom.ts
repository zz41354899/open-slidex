...
type UseCanvasPanZoomArgs = {
  ...
  preserveNativeScrollPosition?: boolean;
...
export function useCanvasPanZoom({
  ...
  closeContextMenu,
  onSetZoomLevel,
  preserveNativeScrollPosition = false,
  resetFramePreview,
  scrollAreaRef,
  ...
  useLayoutEffect(() => {
    ...
    if (!preserveNativeScrollPosition) {
    ...
    if (preserveNativeScrollPosition) {
      scrollArea.scrollLeft += correction.x;
      scrollArea.scrollTop += correction.y;
      return;
    }
  ...
  }, [actualScale, canvasRef, canvasViewportOffset.x, canvasViewportOffset.y, preserveNativeScrollPosition, scrollAreaRef, setCanvasViewportOffset]);
  ...
  function handleNativeCanvasToolWheel(event: globalThis.WheelEvent) {
    const isModifierZoom = isModifierCanvasZoomEvent(event);
    if (event.deltaY === 0 || (activeCanvasTool !== "zoom" && !isModifierZoom)) return;
    event.preventDefault();
    event.stopPropagation();
  ...
  function bufferCanvasZoomAtPoint(point: { clientX: number; clientY: number }, targetScale: number) {
    ...
    if (Math.abs(scrollNormalization.x) > zoomAnchorEpsilon || Math.abs(scrollNormalization.y) > zoomAnchorEpsilon) {
      if (preserveNativeScrollPosition) {
        scrollArea.scrollLeft += scrollNormalization.x;
        scrollArea.scrollTop += scrollNormalization.y;
      } else {
        setCanvasViewportOffset((current) => clampCanvasViewportOffset({
