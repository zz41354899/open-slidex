...
export function ImageCropEditor({
  ...
  function startPan(event: ReactPointerEvent<HTMLDivElement>) {
    ...
    if (!rect) return;
    const { scaleX, scaleY } = normalizedImageScales("cover", mediaProps.scaleX, mediaProps.scaleY);
...
function cropPreviewStyles({
  ...
  const normalizedScales = normalizedImageScales(fit, scaleX, scaleY);
  return {
    image: {
    ...
    transform: {
      transform: `translate(${cropX}%, ${cropY}%) scale(${normalizedScales.scaleX}, ${normalizedScales.scaleY})`,
