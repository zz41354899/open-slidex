          ...
          <div className="flex flex-col gap-2 rounded-xl border border-white/[0.06] bg-black/25 p-2.5">
            {block.props.shapeImageSrc ? (
                ...
                <div className="absolute right-1.5 top-1.5 flex gap-1 rounded-lg bg-black/65 p-1 backdrop-blur">
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <label aria-label={tx("Replace image")} className="flex h-7 w-7 cursor-pointer items-center justify-center rounded-md text-neutral-200 outline-none hover:bg-white/10 focus-visible:ring-1 focus-visible:ring-violet-300/60">
                        <Upload size={13} />
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        aria-label={tx("Remove image fill")}
                        className="h-7 w-7 rounded-md text-rose-300 hover:bg-rose-500/15 hover:text-rose-200"
                        onClick={() => { if (requestImageRemoval()) void removeImageForBlock(selectedBlockIndex); }}
                        size="icon-xs"
                        type="button"
                        variant="ghost"
                      >
                    ...
                    <TooltipContent side="left">{tx("Remove image fill")}</TooltipContent>
                  </Tooltip>
                </div>
              </div>
            ) : (
              <label className="flex h-20 cursor-pointer flex-col items-center justify-center gap-1.5 rounded-lg border border-dashed border-white/[0.1] text-neutral-500 transition hover:border-violet-400/40 hover:text-violet-200">
                <ImagePlus size={19} />
                <span className="text-[11px] font-semibold">{tx("Place image in shape")}</span>
                <input accept="image/*" className="sr-only" onChange={(event) => { uploadImageForBlock(selectedBlockIndex, event.target.files?.[0]); event.currentTarget.value = ""; }} onClick={(event) => { if (!requestImageUpload()) event.preventDefault(); }} type="file" />
              </label>
            )}
            {block.props.shapeImageSrc ? (
              <>
                <NativeSelect
                  ...
                  options={[{ label: "Crop to fill", value: "cover" }, { label: "Fit image", value: "contain" }]}
                ...
                <div className="grid grid-cols-2 gap-1.5">
                  ...
                  <NumberInput min="0.1" max="8" onChange={(value) => updateBlock(selectedBlockIndex, { ...block.props, shapeImageScaleX: value === "" ? 1 : value, shapeImageScaleY: value === "" ? 1 : value })} prefix={<span className="text-[9px]">{tx("Zoom")}</span>} step="0.05" value={block.props.shapeImageScaleX ?? 1} />
                  <button className="rounded-lg border border-white/[0.06] bg-black/30 text-[10px] font-semibold text-neutral-400 hover:bg-white/[0.05] hover:text-white" onClick={() => updateBlock(selectedBlockIndex, { ...block.props, shapeImageCropX: 0, shapeImageCropY: 0, shapeImageScaleX: 1, shapeImageScaleY: 1 })} type="button">{tx("Reset crop")}</button>
                </div>
              </>
            ) : null}
          </div>
