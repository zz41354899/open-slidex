            ...
            row.map((cell, columnIndex) => {
              ...
              return (
                <EditableTableCell
                  ...
                  rowIndex={rowIndex}
                  override={tableCellStyleOverride(block.props, rowIndex, columnIndex)}
                  selected={selected}
                  value={cell}
                />
              );
...
function EditableTableCell({
  ...
  rowIndex: number;
  override: CellStyleOverride;
  selected: boolean;
  value: string;
}) {
  const overrideBg = override.background;
  const overrideText = override.textColor;
  const overrideBorder = override.borderColor;
  const overrideFontSize = override.fontSize;
