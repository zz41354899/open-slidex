...
import { colorPlanePosition, colorPlaneValueAtPoint } from "@/features/pitch/application/colorPlaneGeometry";
import { colorSwatchStyle } from "@/features/pitch/ui/inspector/color/colorSwatchStyle";
import { colorPresetName, defaultColorPresets } from "@/features/pitch/ui/inspector/color/palettes";
...
export function ColorPickerSurface({
  ...
  return (
    <div
      aria-label={`${localizedLabel} ${tx("color controls")}`}
      className="space-y-3"
        ...
        <>
          <div className="flex items-center gap-2">
            ...
            <div className="flex min-w-0 flex-1 items-center overflow-hidden rounded-lg border border-white/[0.08] bg-black/30 focus-within:border-[#8ea5ff]/55">
              ...
              <input
                aria-label={`${localizedLabel} ${tx("hex value")}`}
          ...
          <div className="grid grid-cols-3 gap-1.5">
            {(["r", "g", "b"] as const).map((channel) => (
              <label className="flex h-8 items-center rounded-lg border border-white/[0.08] bg-black/30 px-2 focus-within:border-[#8ea5ff]/55" key={channel}>
                ...
                <input
                  aria-label={`${localizedLabel} ${channel.toUpperCase()}`}
      ...
      <div>
        ...
        <div className="grid grid-cols-8 gap-1.5">
          {resolvedPresets.map((preset) => {
            const colorName = colorPresetName(preset);

            return (
              <button
              aria-label={`${tx("Use color")} ${colorName ? `${tx(colorName)} ` : ""}${preset}`}
              ...
              title={colorName ? `${tx(colorName)} · ${preset.toUpperCase()}` : preset.toUpperCase()}
              type="button"
              />
            );
          })}
...
function ColorPlane({
  ...
  function update(event: PointerEvent<HTMLDivElement>) {
    ...
    if (!rect) return;
    onChange(colorPlaneValueAtPoint({
      height: rect.height,
      width: rect.width,
  ...
  const pointerPosition = colorPlanePosition(hsv);
  ...
  return (
    <div
      ...
      aria-valuetext={tx("Saturation {saturation}%, brightness {brightness}%", {
        brightness: Math.round(hsv.v * 100),
        saturation: Math.round(hsv.s * 100)
      })}
      className={`relative h-32 touch-none overflow-visible rounded-lg border outline-none transition-[border-color,box-shadow] ${
        isDragging
          ? "cursor-grabbing border-neutral-300 shadow-[0_0_0_3px_rgba(255,255,255,0.08)]"
          : "cursor-crosshair border-neutral-600 hover:border-neutral-400 focus-visible:border-neutral-300 focus-visible:shadow-[0_0_0_3px_rgba(255,255,255,0.08)]"
      }`}
    ...
    >
      <span
        ...
        style={{ backgroundColor: `hsl(${hsv.h} 100% 50%)`, borderRadius: "inherit" }}
      >
        <span
      ...
      <span
        className={`pointer-events-none absolute z-10 h-5 w-5 -translate-x-1/2 -translate-y-1/2 rounded-full border-[3px] border-white bg-transparent shadow-[0_1px_8px_rgba(0,0,0,0.9)] transition-transform ${
...
function HueSlider({ hue, onChange }: { hue: number; onChange: (value: number) => void }) {
  ...
  return (
    <div
      ...
      aria-valuetext={tx("{degrees} degrees", { degrees: Math.round(hue) })}
