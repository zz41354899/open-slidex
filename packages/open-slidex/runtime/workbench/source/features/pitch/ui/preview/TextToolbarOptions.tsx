...
export function TextColorPopover({
  ...
  function positionPanel() {
    ...
    if (!button) return;
    const optionsPanel = button.closest<HTMLElement>("[data-text-options-popover]");
    const anchorRect = (optionsPanel ?? button).getBoundingClientRect();
    const panelWidth = panelRef.current?.offsetWidth ?? 250;
    const panelHeight = panelRef.current?.offsetHeight ?? 400;
    const gap = 8;
    const padding = 12;
    const fitsRight = anchorRect.right + gap + panelWidth <= window.innerWidth - padding;
    const left = fitsRight
      ? anchorRect.right + gap
      : Math.max(padding, anchorRect.left - gap - panelWidth);
    const top = Math.min(
      Math.max(anchorRect.top, padding),
      Math.max(padding, window.innerHeight - padding - panelHeight)
    );
  ...
  useEffect(() => {
    ...
    window.addEventListener("mousemove", positionPanel);
    document.addEventListener("pointerdown", closeOnOutsidePointer, true);
    document.addEventListener("keydown", closeOnEscape);
    return () => {
      window.removeEventListener("resize", positionPanel);
      window.removeEventListener("scroll", positionPanel, true);
      window.removeEventListener("mousemove", positionPanel);
  ...
  return (
    <div className="relative">
      ...
      {isOpen && typeof document !== "undefined" ? createPortal(
        <div
          aria-label={tx("Text color panel")}
          className="fixed z-[140] max-h-[calc(100vh-24px)] w-[250px] overflow-y-auto rounded-xl border border-white/10 bg-[#17171a] p-3 shadow-[0_24px_70px_rgba(0,0,0,0.58)]"
          data-text-edit-control
          data-text-options-floating-child
