...
export function motionDocLineHeightCanvasValue(
  lineHeight: string | number | undefined,
  lineHeightPt: string | number | undefined,
...
export function motionDocLineHeightCanvasPixels(
  ...
  const exactPoints = positiveNumber(lineHeightPt);
  if (exactPoints !== undefined) {
    return motionDocFontPointsToCanvasPixels(exactPoints);
  }

  return motionDocFontPointsToCanvasPixels(fontSizePoints) * (positiveNumber(lineHeight) ?? defaultLineHeight);
}

export const OFFICE_LINE_HEIGHT_PRESETS = [1, 1.2, 1.5, 2] as const;

export const OFFICE_CHARACTER_SPACING_PRESETS = [-0.5, 0, 0.5] as const;

export function matchingOfficeSpacingPreset(
  value: number,
  presets: readonly number[],
  tolerance = 0.001
) {
  return presets.find((preset) => Math.abs(value - preset) <= tolerance);
}
