...
export function insertTableRow(props: MotionDocProps, rowIndex: number): MotionDocProps {
  ...
  return withTableSize(
    { ...props, cellOverrides: serializeCellOverrides(reindexCellOverrides(parseCellOverrides(props), "row", insertIndex, 1)) },
    size.rows + 1,
    size.columns,
    cells,
    { rowHeights }
...
export function deleteTableRow(props: MotionDocProps, rowIndex: number): MotionDocProps {
  ...
  return withTableSize(
    { ...props, cellOverrides: serializeCellOverrides(reindexCellOverrides(parseCellOverrides(props), "row", deleteIndex, -1)) },
    size.rows - 1,
    size.columns,
    cells,
    { rowHeights }
...
export function insertTableColumn(props: MotionDocProps, columnIndex: number): MotionDocProps {
  ...
  return withTableSize(
    { ...props, cellOverrides: serializeCellOverrides(reindexCellOverrides(parseCellOverrides(props), "column", insertIndex, 1)) },
    size.rows,
    size.columns + 1,
    cells,
    { columnWidths }
...
export function deleteTableColumn(props: MotionDocProps, columnIndex: number): MotionDocProps {
  ...
  return withTableSize(
    { ...props, cellOverrides: serializeCellOverrides(reindexCellOverrides(parseCellOverrides(props), "column", deleteIndex, -1)) },
    size.rows,
    size.columns - 1,
    cells,
    { columnWidths }
...
function withTableSize(
  ...
  return {
    ...props,
    cellOverrides: serializeCellOverrides(normalizeCellOverrides(parseCellOverrides(props), rows, columns)),
...
export type CellStyleOverrides = Record<string, CellStyleOverride>;
...
export function parseCellOverrides(props: MotionDocProps): CellStyleOverrides {
  return parseCellOverridesJson(props.cellOverrides);
}

/** Per-cell overrides take priority over a row, column, and table defaults. */
export function tableCellStyleOverride(props: MotionDocProps, rowIndex: number, columnIndex: number): CellStyleOverride {
  return {
...
export function updateCellOverride(
  props: MotionDocProps,
  rowIndex: number,
  columnIndex: number,
  ...
  if (rowIndex < 0 || rowIndex >= rows || columnIndex < 0 || columnIndex >= columns) return props;

  const overrides = parseCellOverrides(props);
  const key = cellOverrideKey(rowIndex, columnIndex);
  overrides[key] = { ...overrides[key], ...patch };
  cleanEmptyCellOverride(overrides, key);
  return { ...props, cellOverrides: serializeCellOverrides(overrides) };
}
...
export function clearCellOverride(props: MotionDocProps, rowIndex: number, columnIndex: number): MotionDocProps {
  const overrides = parseCellOverrides(props);
  delete overrides[cellOverrideKey(rowIndex, columnIndex)];
  return { ...props, cellOverrides: serializeCellOverrides(overrides) };
}
...
export function serializeCellOverrides(overrides: CellStyleOverrides): string {
  const keys = Object.keys(overrides);
  if (keys.length === 0) return "";
  return JSON.stringify(overrides);
}
...
function parseCellOverridesJson(value: string | number | undefined): CellStyleOverrides {
  if (typeof value !== "string" || !value.trim()) return {};
  try {
    const parsed = JSON.parse(value);
    if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) return {};
    return Object.fromEntries(
      Object.entries(parsed).filter(([key, override]) =>
        Boolean(parseCellOverrideKey(key)) && typeof override === "object" && override !== null && !Array.isArray(override)
      )
...
function cleanEmptyCellOverride(overrides: CellStyleOverrides, key: string) {
  const entry = overrides[key];
  if (!entry) return;
  const hasValue = Object.values(entry).some((value) =>
    (typeof value === "string" && value.trim() !== "") || typeof value === "number"
  );
  if (!hasValue) delete overrides[key];
}

function cellOverrideKey(rowIndex: number, columnIndex: number) {
  return `${rowIndex}:${columnIndex}`;
}

function parseCellOverrideKey(key: string) {
  const match = /^(\d+):(\d+)$/.exec(key);
  if (!match) return null;
  return { columnIndex: Number(match[2]), rowIndex: Number(match[1]) };
}

function normalizeCellOverrides(overrides: CellStyleOverrides, rows: number, columns: number): CellStyleOverrides {
  return Object.fromEntries(
    Object.entries(overrides).filter(([key]) => {
      const coordinate = parseCellOverrideKey(key);
      return Boolean(coordinate && coordinate.rowIndex < rows && coordinate.columnIndex < columns);
    })
  );
}

function reindexCellOverrides(
  overrides: CellStyleOverrides,
  axis: "row" | "column",
  pivot: number,
  delta: -1 | 1
): CellStyleOverrides {
  const next: CellStyleOverrides = {};

  for (const [key, override] of Object.entries(overrides)) {
    const coordinate = parseCellOverrideKey(key);
    if (!coordinate) continue;
    const index = axis === "row" ? coordinate.rowIndex : coordinate.columnIndex;
    if (delta < 0 && index === pivot) continue;
    if (index >= pivot) {
      if (axis === "row") coordinate.rowIndex += delta;
      else coordinate.columnIndex += delta;
    }
    next[cellOverrideKey(coordinate.rowIndex, coordinate.columnIndex)] = override;
  }
