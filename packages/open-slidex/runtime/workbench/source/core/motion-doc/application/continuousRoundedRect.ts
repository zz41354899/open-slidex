...
export function normalizedRelativeCornerRadii(
  corner: number,
  frameWidth: number,
  frameHeight: number
) {
...
export function relativeCornerFromRadius(radius: number, frameWidth: number, frameHeight: number) {
  const shortEdge = Math.max(Math.min(frameWidth, frameHeight), 0);
  if (shortEdge <= 0) return 0;
  return Number((clamp(radius / (shortEdge / 2) * 50, 0, 50)).toFixed(1));
}
