  ...
  return (
    <div className="osx-settings-view">
      ...
      <section className="osx-settings-card is-mcp">
        <header><span className="osx-settings-icon"><Cable size={19} /></span><div><small>{zh ? "全域整合" : "Global integration"}</small><strong>{presentationScopedMcp ? (zh ? "簡報 MCP 伺服器" : "Presentation MCP Server") : (zh ? "工作區 MCP 伺服器" : "Workspace MCP Server")}</strong></div></header>
        <p>{presentationScopedMcp
          ? (zh ? "不啟動或偵測任何 CLI。這份使用者層級設定只允許 Desktop Agent 讀寫目前安裝資料夾內的 presentation.mdx。" : "No CLI is launched or detected. This user-level configuration only allows desktop agents to work with presentation.mdx in this installed project folder.")
          : (zh ? "不啟動或偵測任何 CLI。這裡只產生一份使用者層級設定，讓 Desktop Agent 直接列出工作區簡報並讀寫各自的 presentation.mdx。" : "No CLI is launched or detected. This generates one user-level configuration so desktop agents can list workspace presentations and work directly with each presentation.mdx.")}</p>
        ...
        {mcpError ? <p className="osx-mcp-error">{mcpError}</p> : mcpSetup ? <>
          <div className="osx-mcp-meta"><span>{zh ? "全域設定位置" : "Global config location"}<code>{mcpSetup.configPath}</code></span><span>{presentationScopedMcp ? (zh ? "簡報路徑" : "Presentation path") : (zh ? "工作區範圍" : "Workspace scope")}<code>{mcpSetup.presentationPath ?? mcpSetup.scopeRoot}</code></span></div>
