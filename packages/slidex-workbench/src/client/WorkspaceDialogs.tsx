      ...
      {importOpen ? (
        <div className="osx-workspace-overlay" onMouseDown={() => !importPending && setImportOpen(false)} role="presentation">
          <section aria-modal="true" className="osx-import-dialog" onMouseDown={(event) => event.stopPropagation()} role="dialog">
            ...
            <p>{zh ? "TSX 是可編輯來源；legacy MDX 匯入後會轉成 TSX。完整資料夾中的引用圖片會一起匯入，PNG 會轉成 WebP。" : "TSX is editable; legacy MDX is converted to TSX on import. Referenced folder images are imported and PNG becomes WebP."}</p>
            <input accept=".tsx,.mdx,.html,text/typescript,text/mdx,text/markdown,text/html" className="sr-only" onChange={(event) => chooseImportFile(event.currentTarget.files?.[0])} ref={importInputRef} type="file" />
            <input {...({ webkitdirectory: "" } as InputHTMLAttributes<HTMLInputElement> & { webkitdirectory: string })} className="sr-only" multiple onChange={(event) => chooseImportFolder(event.currentTarget.files)} ref={importFolderInputRef} type="file" />
            <button className={`osx-mdx-dropzone${importFile ? " has-file" : ""}`} onClick={() => importInputRef.current?.click()} onDragOver={(event: DragEvent<HTMLButtonElement>) => event.preventDefault()} onDrop={(event: DragEvent<HTMLButtonElement>) => { event.preventDefault(); chooseImportFile(event.dataTransfer.files[0]); }} type="button">
              {importFile ? <FileCheck2 size={28} /> : <Upload size={28} />}<span><strong>{importFile?.name ?? (zh ? "拖放 TSX、MDX 或 HTML" : "Drop TSX, MDX, or HTML")}</strong><small>{importFile ? formatBytes(importFile.size) : (zh ? ".tsx/.mdx/.html · 上限 50 MB" : ".tsx/.mdx/.html · 50 MB maximum")}</small></span>
            </button>
            <button className="osx-import-folder" onClick={() => importFolderInputRef.current?.click()} type="button"><FolderInput size={16} /><span>{zh ? "選擇完整 TSX、MDX 或 HTML 簡報資料夾" : "Choose complete TSX, MDX, or HTML presentation folder"}</span><small>{importSidecars.length ? (zh ? `已收集 ${importSidecars.length} 個素材；PNG 將轉成 WebP` : `${importSidecars.length} sidecars collected; PNG will become WebP`) : (zh ? "自動收集引用圖片並將 PNG 轉成 WebP" : "Collect referenced images and convert PNG to WebP automatically")}</small></button>
            <div className="osx-import-policy"><ShieldCheck size={15} /><span>{zh ? "HTML 必須可離線使用；請選擇完整資料夾以匯入引用圖片，HTTP(S) 線上素材無法匯入。" : "HTML must work offline. Choose the complete folder to include referenced images; HTTP(S) resources cannot be imported."}</span></div>
