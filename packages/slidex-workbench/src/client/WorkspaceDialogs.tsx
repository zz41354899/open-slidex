      ...
      {importOpen ? (
        <div className="osx-workspace-overlay" onMouseDown={() => !importPending && setImportOpen(false)} role="presentation">
          <section aria-modal="true" className="osx-import-dialog" onMouseDown={(event) => event.stopPropagation()} role="dialog">
            ...
            <p>{zh ? "MDX 保留原生可編輯圖層；MDX 或 HTML 都可選擇完整資料夾，引用的圖片會一起匯入，PNG 會轉成 WebP。" : "MDX keeps native editable layers. Choose the complete MDX or HTML folder to import referenced images with it; PNG is converted to WebP."}</p>
