...
commands require Node.js 22.12 or newer; Bun uses its own current runtime.
OpenSlideX supports macOS and Linux. The no-Node standalone installer is
...
The installer checks each release archive against its SHA-256 checksum before
replacing the active version. Release archives include an SPDX SBOM. GitHub CLI
is not required.
