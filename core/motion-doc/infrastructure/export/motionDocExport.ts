...
import { percentFrameValue } from "@/core/motion-doc/domain/frame";
import {
  parseColOverrides,
  parseRowOverrides,
  tableCellsFromProps,
...
function renderMotionBlock(block: MotionDocBlock, content: string) {
  ...
  const frameAttributes = isPositionedProps(props)
    ? ` data-slidex-x="${framePercent(props.x, 8)}" data-slidex-y="${framePercent(props.y, 12)}" data-slidex-w="${framePercent(props.w, 42)}" data-slidex-h="${framePercent(props.h, 18)}"`
    : "";
...
function renderShapeSvg(props: MotionDocProps, blockIndex: number) {
  ...
  const imageLayer = shapeImageSrc && shape !== "line"
    ? `<image clip-path="url(#${imageClipId})" href="${escapeAttribute(shapeImageSrc)}" preserveAspectRatio="${shapeImageFit === "fill" ? "none" : shapeImageFit === "contain" ? "xMidYMid meet" : "xMidYMid slice"}" x="${50 - 50 * shapeImageScaleX + shapeImageCropX}" y="${50 - 50 * shapeImageScaleY + shapeImageCropY}" width="${100 * shapeImageScaleX}" height="${100 * shapeImageScaleY}" />`
    : "";
