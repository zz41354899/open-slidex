...
export function ShapeBlock({
  ...
  const maskId = useId();
  const shapeClipId = `${maskId}-shape-image`;
        ...
        <g mask={normalizedMask === "none" ? undefined : `url(#${maskId})`}>
          {localShapeImageSource && normalizedShape !== "line" ? (
            <image
              ...
              preserveAspectRatio={shapeImageFit === "fill" ? "none" : shapeImageFit === "contain" ? "xMidYMid meet" : "xMidYMid slice"}
              style={{
                transform: `translate(${normalizeNumber(shapeImageCropX, 0)}px, ${normalizeNumber(shapeImageCropY, 0)}px) scale(${normalizeNumber(shapeImageScaleX, 1)}, ${normalizeNumber(shapeImageScaleY, 1)})`,
