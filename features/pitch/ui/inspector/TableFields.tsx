...
export function TableFields({ block, selectedBlockIndex, updateBlock }: BlockFieldProps) {
  ...
  const selectionLabel = hasSelection
    ? selectionKind === "row"
      ? (locale === "zh-TW" ? `第 ${selectionIndex! + 1} 列` : `Row ${selectionIndex! + 1}`)
      : (locale === "zh-TW" ? `第 ${columnLabel(selectionIndex!)} 欄` : `Column ${columnLabel(selectionIndex!)}`)
    : tx("Entire table");
        ...
        <div className="flex items-center justify-between">
          ...
          {hasSelection ? (
            <button
              ...
              title={tx("Reset to table default")}
              type="button"
            >
              <X size={10} />
              {tx("Reset")}
  ...
  return (
    <>
      ...
      {Array.from({ length: columns }, (_, columnIndex) => {
        ...
        return (
          <input
            aria-label={locale === "zh-TW" ? `儲存格 ${columnLabel(columnIndex)}${rowIndex + 1}` : `Cell ${columnLabel(columnIndex)}${rowIndex + 1}`}
    ...
    <div className="flex flex-col gap-1.5">
      ...
      {isOpen && typeof document !== "undefined" ? createPortal(
        ...
        >
          ...
          <CompactColorPanel label={tx(label)} onChange={onChange} placeholder={placeholder} value={color} />
        </div>,
        document.body
      ) : null}
    </div>
