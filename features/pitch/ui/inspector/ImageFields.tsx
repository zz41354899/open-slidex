          ...
          {hasImage ? (
            <>
              ...
              <div className="absolute right-2 top-2 z-10 flex items-center gap-1 rounded-[10px] border border-white/10 bg-black/55 p-1 shadow-lg backdrop-blur-md">
                <label aria-label={tx("Replace image")} className="flex h-7 w-7 cursor-pointer items-center justify-center rounded-lg text-neutral-300 outline-none transition hover:bg-white/10 hover:text-white active:scale-90" title={tx("Replace image")}>
                  <Upload size={14} />
                  <input
                    accept="image/*"
                    className="sr-only"
              ...
              {isRemoveConfirmationOpen ? (
                <div className="absolute inset-x-2 bottom-2 z-20 flex items-center gap-2 rounded-xl border border-white/10 bg-[#111114]/95 p-2 shadow-[0_12px_30px_rgba(0,0,0,0.45)] backdrop-blur-xl">
                  ...
                  <button aria-label={tx("Cancel remove")} className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg text-neutral-500 transition hover:bg-white/[0.07] hover:text-white" onClick={() => setIsRemoveConfirmationOpen(false)} title={tx("Cancel")} type="button"><X size={13} /></button>
                  <button aria-label={tx("Confirm remove")} className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-rose-500/15 text-rose-300 transition hover:bg-rose-500/25 hover:text-rose-200 active:scale-90" onClick={removeImage} title={tx("Remove")} type="button"><Trash2 size={13} /></button>
                </div>
              ) : null}
            </>
          ) : (
