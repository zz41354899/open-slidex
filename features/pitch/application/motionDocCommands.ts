...
function blankSlideSourceFromReference(source: string, slideIndex: number) {
  const { accent, background, textColor, theme } = referenceSlideStyle(source, slideIndex);
  const txtAttr = textColor ? ` textColor="${textColor}"` : "";
  return `<Slide canvasHeight={${MOTION_DOC_CANVAS_PROPS.canvasHeight}} canvasWidth={${MOTION_DOC_CANVAS_PROPS.canvasWidth}} fontSizeUnit="${MOTION_DOC_CANVAS_PROPS.fontSizeUnit}" duration={5} theme="${theme}" background="${background}" accent="${accent}"${txtAttr}>\n</Slide>`;
}
